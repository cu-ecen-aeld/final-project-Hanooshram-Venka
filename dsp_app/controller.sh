...
while true; do
    ...
    case $cmd in
        tempo)
            ...
            ;;
        pitch)
            # 1.  sends the command to the Pi to trigger C++ logs
            echo "set pitch $arg1" | nc -w 1 $PI_IP $PORT

            # 2. bounds check to ensure controller dashboard accuracy
            is_valid=$(awk -v val="$arg1" 'BEGIN {print (val >= 0.5 && val <= 2.0)}')
            if [ "$is_valid" -eq 1 ]; then
                CURRENT_PITCH=$arg1
            else
        ...
        loop)
            if [ "$arg1" == "off" ]; then
                ...
                # 1. sends atomic command to Pi to trigger C++ logs
                echo "set loop $arg1 $arg2" | nc -w 1 $PI_IP $PORT

                # 2. Dashboard check (A >= 0 and A < B)
