 ...
 * #9 - A-to-B Audio Looping Logic (Newly added for Issue #9)
 *
 * Brief:
 * - Implements multithreaded WAV playback with SoundTouch DSP.
 * - Adds dynamic A-to-B looping capabilities. The Producer thread monitors the
 * current read frame and uses sf_seek() to seamlessly rewind to Point A when it
 * hits Point B, creating gapless audio loops based on socket IPC (inter-process communication)
 *
 */
...
// Global audio metadata for time-to-frame conversions
    ...
    while (!ring_buf.eof_reached) {
        ...
        if (valread > 0) {
            ...
            if (sscanf(buffer, "set %s %f", param, &value) == 2) {
                ...
                } else if (strcmp(param, "loop_a") == 0) {
                    shared_config.loop_a_sec = value;
                    printf("[Control] Loop Point A set to %.2f seconds\n", shared_config.loop_a_sec);
                } else if (strcmp(param, "loop_b") == 0) {
                    shared_config.loop_b_sec = value;
                    printf("[Control] Loop Point B set to %.2f seconds\n", shared_config.loop_b_sec);
                } else if (strcmp(param, "loop") == 0) {
                    shared_config.loop_enabled = (value >= 1.0f);
                    printf("[Control] A-to-B Looping is now %s\n", shared_config.loop_enabled ? "ON" : "OFF");
                } else {
...
int main(int argc, char *argv[]) {
    ...
    // Save sample rate globally for Time-to-Frame math
    ...
    // Default Point B to the very end of the file
    shared_config.loop_b_sec = (float)sfinfo.frames / sfinfo.samplerate; 
